...
KANBAN_GUIDANCE = (
    ...
    "backend (local/docker/modal/ssh). MCP runtimes may show prefixed names "
    "like `mcp__hermes-tools__kanban_show` or "
    "`mcp__hermes-tools__kanban_block`; use those.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    ...
    "reviewer can approve+unblock or request changes. If they reply "
    "`approved` in the notification thread, the dispatcher will unblock and "
    "respawn you; on the next run, read the comment thread and immediately "
    "call `kanban_complete` using the handoff you already wrote. Reviewing-then-"
