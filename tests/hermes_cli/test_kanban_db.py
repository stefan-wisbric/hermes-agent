...
def test_connect_reindexes_rebuildable_event_run_index_mismatch(tmp_path, monkeypatch):
    """An integrity failure limited to idx_events_run should self-heal.

    SQLite can report ``wrong # of entries in index ...`` for a secondary
    index while the table data remains intact. Rebuilding that index is
    safer than refusing the board and does not overwrite task data.
    """
    db_path = tmp_path / "kanban.db"
    ...
    class ProbeConnection:
        ...
        def execute(self, sql, *args, **kwargs):
            ...
            if normalized == "PRAGMA INTEGRITY_CHECK":
                if reindexed:
                    return _Rows([("ok",)])
                return _Rows([("wrong # of entries in index idx_events_run",)])
            if normalized == "REINDEX IDX_EVENTS_RUN":
                reindexed = True
                return _Rows([])
    ...
    def connect_spy(*args, **kwargs):
        ...
        if calls == 1:
            return ProbeConnection(conn)
        return conn

    monkeypatch.setattr(kb.sqlite3, "connect", connect_spy)

    with kb.connect(db_path=db_path) as conn:
        tid = kb.create_task(conn, title="still here")
        assert kb.get_task(conn, tid).title == "still here"

    assert reindexed
    assert any(sql.strip().upper() == "REINDEX IDX_EVENTS_RUN" for sql in executed)
    assert not list(tmp_path.glob("*.corrupt.*"))


def test_connect_reindexes_multiple_known_index_integrity_errors(tmp_path, monkeypatch):
    """All-known index drift should rebuild instead of backing up the DB."""
    db_path = tmp_path / "kanban.db"
    kb.init_db(db_path=db_path)
    kb._INITIALIZED_PATHS.discard(str(db_path.resolve()))
    ...
    integrity_errors = [
        ...
        ("row 48 missing from index idx_tasks_status",),
        ("row 48 missing from index idx_tasks_assignee_status",),
    ]
        ...
        def execute(self, sql, *args, **kwargs):
            ...
            if normalized == "PRAGMA INTEGRITY_CHECK":
                if {
                    ...
                    return _Rows([("ok",)])
                return _Rows(integrity_errors)
            if normalized.startswith("REINDEX "):
                rebuilt.add(str(sql).split(None, 1)[1])
                return _Rows([])
            return self._conn.execute(sql, *args, **kwargs)
    ...
    with kb.connect(db_path=db_path) as conn:
        tid = kb.create_task(conn, title="indexes rebuilt")
        assert kb.get_task(conn, tid).title == "indexes rebuilt"
